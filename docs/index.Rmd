...
---
# Introduction 
Seasonal Affective Disorder (SAD) is a mood disorder characterized by recurring episodes of depression that occur at specific times of the year, typically in autumn and winter. It is believed that SAD is linked to seasonal variations in daylight exposure, affecting patients well-being and mental health.  

In Scotland, daylight hours fluctuate significantly throughout the year. Therefore, understanding the relationship between SAD and prescription trends of antidepressants is of public health interests. This analysis will focus on one type of antidepressants, called Selective Serotonin Re-uptake Inhibitors (SSRIs), commonly prescribed for this condition (NHS reference).  
...
seasonal_summary_gt <- scotland_seasonal_summary %>%
  ...
  # Format the "Total Prescriptions" column with no decimals
  fmt_number(
    columns = "total_prescriptions",
    decimals = 0  # No decimals
  ) %>%
  # Format the "Avg. Sunshine Hours" column with 3 decimals
  fmt_number(
    columns = "avg_sunshine_hours",
    decimals = 2  # 2 decimals for precision
  ) %>%
  ...
  # Add a spanner with updated text
  tab_spanner(
  ...
  tab_style(
    style = list(
      ...
      cell_text(color = "white", weight = "bold")      # Bold text for emphasis
    ),
